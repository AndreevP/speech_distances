"""
Based on https://github.com/SeanNaren/deepspeech.pytorch
"""
import math
...
class MaskConv(nn.Module):
    def __init__(self, seq_module):
        ...
        This is to ensure that the results of the model do not change when
...
class Lookahead(nn.Module):
    """
    Wang et al 2016 - "Lookahead Convolution Layer for Unidirectional Recurrent
     Neural Networks"
...
class DeepSpeech(nn.Module):
    def __init__(
        ...
        # Based on above convolutions and spectrogram size using
    ...
    def serialize_state(self):
        return {
            ...
            "rnn_type": supported_rnns_inv.get(
                self.rnn_type, self.rnn_type.__name__.lower()
            ),
...
class DeepSpeechEncoder(DeepSpeech):
    ...
    def load_model(cls, path):
        ...
        model = DeepSpeechEncoder.load_model_package(package)
        return model
...
class DeepSpeechInputFeatureExtractor(nn.Module):
    ...
    def __init__(self, n_fft=320, w_len=320, h_len=160, normalize=True, eps=1e-12):
        super().__init__()
    ...
    def forward(self, input_signal, length=None):
        ...
        f = torch.stft(input_signal, n_fft=self.n_fft, hop_length=self.h_len, win_length=self.w_len, window=w)
...
class DeepSpeechEncoderWrapper(nn.Module):
    """
...
def _collate_fn(batch):
    ...
    input_sizes = torch.LongTensor(minibatch_size)
    ...
    return (inputs, input_sizes), targets
